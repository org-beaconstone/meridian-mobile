...
@main struct LiveChecks {
  static func main() async throws {
    ...
    guard !options.isEmpty, options.count==catalog.providers.reduce(0, {$0+$1.methods.count}) else {throw CheckError.failed("provider catalog")}
    let methodId=options[0].methodId
    let key=UUID().uuidString
    let payment=try await api.submitPayment(recipientId:"northline-studio",amountMinor:2599,method:methodId,note:"Swift native transport check",idempotencyKey:key)
    guard payment.ok, payment.state?.balance==1245451, payment.transaction != nil else {throw CheckError.failed("payment response")}
    let duplicate=try await api.submitPayment(recipientId:"northline-studio",amountMinor:2599,method:methodId,note:"Swift native transport check",idempotencyKey:key)
    guard duplicate.ok, duplicate.state?.balance==1245451 else {throw CheckError.failed("idempotency")}
    let pending=try await api.submitPayment(recipientId:"northline-studio",amountMinor:100,method:methodId,note:"pending",scenario:.pending,idempotencyKey:UUID().uuidString)
