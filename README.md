...
With the Android catalog flag off (the default), payment methods stay hardcoded to Adyen/card and Worldpay/bank in the native UI, matching the previous picker. Set `MERIDIAN_CATALOG_CONFIG_DRIVEN=true` or `-Dmeridian.catalog.configDriven=true` before connect to resolve the Android method picker from `GET /catalog`. That configuration is cached in memory for 30 seconds and falls back to the last known good catalog when the fetch fails or the body is unusable. The recognized methods remain Adyen card (`adyen_card`) and Worldpay bank (`worldpay_bank`); submission still sends `card` or `bank` with the same idempotency key. No real provider calls, account credentials, SCA, or production authentication exist here. A room ID is a synthetic-data partition, not a security boundary.
