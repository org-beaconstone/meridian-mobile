...
Android's payment picker reads `GET /config/payment-providers` and renders the providers returned for the customer's corridor. The shipped flag is that remote picker. Rollback sets `MERIDIAN_PROVIDER_PICKER=milestone2` (or the in-app Milestone 2 control), which restores the Adyen card and Worldpay bank baseline without a code revert. iOS and the browser companion still use that hardcoded pair. No third provider is named or implemented in the client. No real provider calls, account credentials, SCA, or production authentication exist here. A room ID is a synthetic-data partition, not a security boundary.
